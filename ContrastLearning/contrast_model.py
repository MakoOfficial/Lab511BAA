...
class Student_Contrast_Model_Pretrain(nn.Module):
    ...
    def downStream(self, image, gender):
        ...
        return contrast_feature, cls_token2, cls_token3, attn0, attn1, attn2, attn3


class Student_Contrast_Model_Pretrain_Vit(nn.Module):
    def __init__(self, backbone):
        super(Student_Contrast_Model_Pretrain_Vit, self).__init__()
...
class Student_Contrast_Model_Pretrain_GCN_Version2(nn.Module):
    def __init__(self, backbone):
        super(Student_Contrast_Model_Pretrain_GCN_Version2, self).__init__()
        self.backbone0 = backbone.backbone0
        self.attn0 = backbone.attn0
        self.backbone1 = backbone.backbone1
        self.attn1 = backbone.attn1
...
class Student_Contrast_Model_End2End(nn.Module):
    def __init__(self, backbone_res):
        ...
        # self.out_channels = out_channels
        self.backbone0 = nn.Sequential(*backbone_res[0:5])
...
class Student_Contrast_Model_Class(nn.Module):
    def __init__(self, backbone, backbone_res):
        super(Student_Contrast_Model_Class, self).__init__()
        self.backbone0 = backbone.backbone0
        self.attn0 = backbone.attn0
        self.backbone1 = backbone.backbone1
        self.attn1 = backbone.attn1
...
def get_student_contrast_model_pretrain_vit(student_path):
    backbone = get_student()
    if student_path is not None:
        backbone.load_state_dict(torch.load(student_path))
...
def get_student_contrast_model_pretrain_gcn_V2(student_path):
    backbone = get_student()
    if student_path is not None:
        backbone.load_state_dict(torch.load(student_path))
...
def get_student_contrast_class_model(student_path):
    backbone = get_student_class()
    if student_path is not None:
        backbone.load_state_dict(torch.load(student_path))

    resnet, output_channels = get_pretrained_resnet50(True)

    return Student_Contrast_Model_Class(backbone, resnet)


def get_student_contrast_model_End2End():
    resnet, output_channels = get_pretrained_resnet50(True)

    return Student_Contrast_Model_End2End(resnet)
