...
from datasets import RSNATrainNoNormDataset, RSNAValidNoNormDataset
...
flags['model_name'] = 'Contrast_WCL_IN_CBAM_AVGPool_AdaA_GenderPlus_Full_1_10_96_Class'
flags['node'] = '训练对比模块，采用分类'
flags['seed'] = 1
...
def train_fn(train_loader, loss_fn, triple_fn, optimizer):
    ...
    for idx, data in enumerate(train_loader):
        ...
        img_gt = data[1].type(torch.FloatTensor).cuda()
...
def training_start(flags):
    ...
    for epoch in range(flags['num_epochs']):
        ...
        log_contrast_losses_to_csv(training_mean_loss, mean_triple_loss_0,
                                   ...
                                   optimizer.param_groups[0]["lr"], os.path.join(save_path, "Contrast_Class_loss.csv"))
        scheduler.step()
...
if __name__ == "__main__":
    ...
    contrast_model = get_student_contrast_class_model(student_path=flags['student_path']).cuda()
    #   load data setting
    ...
    train_set = RSNATrainNoNormDataset(train_df, train_path, flags['img_size'])
    valid_set = RSNAValidNoNormDataset(valid_df, valid_path, flags['img_size'])
    test_set = RSNAValidNoNormDataset(test_df, valid_path, flags['img_size'])
