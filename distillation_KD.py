...
flags['note'] = '蒸馏模块消融，特征蒸馏'
flags['model_name'] = 'KD_Distillation_Feature_KL_4K_t4_s2_t5_s3_1_6'
flags['seed'] = 1
...
def train_fn(train_loader, loss_fn, optimizer):
    ...
    for idx, data in enumerate(train_loader):
        ...
        _, t1, t2, t3, t4, t5, _, _, _, _ = teacher.forward_attention(image)
        class_feature, s1, s2, s3, s4 = student_model(image, gender)
        y_pred = class_feature.squeeze()
        label = label.squeeze()

        loss = loss_fn(y_pred, label)
        train_attn_loss = feature_offset_kl_loss_firstStage(t1, t2, t3, t4, t5, s1, s2, s3, s4)
        # train_attn_loss = feature_offset_mse_loss_firstStage(t1, t2, t3, t4, t5, s1, s2, s3, s4)
        ...
        print(f"batch_loss: {batch_loss}, batch_feature_loss: {batch_attn_loss}, penalty_loss: {penalty_loss.item()}")
...
def evaluate_fn(val_loader):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(val_loader):
            ...
            val_attn_loss = feature_offset_kl_loss_firstStage(t1, t2, t3, t4, t5, s1, s2, s3, s4)
            # val_attn_loss = feature_offset_mse_loss_firstStage(t1, t2, t3, t4, t5, s1, s2, s3, s4)
            ...
            if batch_idx == len(val_loader) - 1:
                save_attn_KD(t2[0], t3[0], t4[0], t5[0], s1[0], s2[0], s3[0], s4[0], save_path)
