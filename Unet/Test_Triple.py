...
if __name__ == '__main__':
    ...
    model_path = '../ckp/Unet_Triple/unet_segmentation_Attn_Unet_RSNA_256_Triple.pth'
    segment_model.load_state_dict(torch.load(model_path), strict=True)
    ...
    data, predict, label, x1, x2, x3, x4, x5, attn1, attn2, attn3, attn4 = eval_pic(segment_model, testDataset, idx=idx)
    _, predict2, label2, _, _, _, _, _, compare1, compare2, compare3, compare4 = eval_pic(compare_model, testDataset,
                                                                                          idx=idx, compare=True)
    # show_a_image(predict, label)
    # visualize_attn_single(data, predict, label, attn1, attn2, attn3, attn4)
